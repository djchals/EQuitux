...
void calculate_EQ(int *arr_hex_pos0,int long_hex_pos0, int *arr_hex_pos1,int long_hex_pos1){
    ...
    float tot_win_pos[9]={};//save in each index the result when this position have the highest score
                        ...
                        if( !(i==k && j==l) &&
                            ...
                            for(m=0;m<long_hex_pos0;m++){
                                for(n=0;n<long_hex_pos1;n++){
                                    ...
                                    combo_hex[1][0]=(*(arr_hex_pos1+m)/0x1000)%0x10;
                                    combo_hex[1][1]=(*(arr_hex_pos1+m)/0x100)%0x10;
                                    combo_hex[1][2]=(*(arr_hex_pos1+m)/0x10)%0x10;
                                    combo_hex[1][3]=*(arr_hex_pos1+m)%0x10;
...
//                                         tot_combos++;
        ...
        tot_combos=tot_win_pos[0]+tot_win_pos[1];
        tot_win_pcent[0]=(tot_win_pos[0]*100)/tot_combos;
        tot_win_pcent[1]=(tot_win_pos[1]*100)/tot_combos;
