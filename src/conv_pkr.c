...
//     char range_pkr[]={"AsAc, A2s, Kx,A2s-A9s,33-66,99"};
    char range_pkr[]={"88"};
//         char range_pkr[]={"As8c"};
    conv_range_pkr_to_hex(range_pkr);
}
int conv_range_pkr_to_hex(char tmp_range[]){
...
//                         printf("i %d %s\n",i,substring);
    ...
    int array_hex_return[cont];//now we make the array with only the combos marked.
    j=0;
    for(i=0;i<MAX_COMBO_HEX;i++){
        if(array_combos_marked_hex[i]!=0){
//             printf("i %x j %d\n",i,j);
            array_hex_return[j]=(int) i;
            j++;
        }
    }
    for(i=0;i<j;i++){
